...
def getNearbyElement(road):
    ...
    sql="select objectid from open.nypd_sectors where st_dwithin(st_geomfromtext('{}',2263),geom,80)".format(road[1])
    curs.execute(sql)
    res=curs.fetchall()
    for line in res:
...
filesql=open(os.path.join(dir_path,'..','data','sql','road2nypdsectors.sql'),'w')
conn = connectDB()
getAllRoads()
