...
def getNearbyElement(road):
    ...
    sql="select objectid from open.nyc_police_incident where st_dwithin(st_geomfromtext('{}',2263),ftus_coord,80)".format(road[1])
    curs.execute(sql)
    res=curs.fetchall()
    for line in res:
...
filesql=open(os.path.join(dir_path,'..','data','sql','road2police_incident.sql'),'w')
conn = connectDB()
getAllRoads()
